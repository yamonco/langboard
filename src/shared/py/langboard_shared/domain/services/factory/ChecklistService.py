...
class ChecklistService(BaseDomainService):
    ...
    def get_api_list_only_by_project(
        self, project: TProjectParam | None, archive_visible_since: SafeDateTime | None = None
    ) -> list[dict[str, Any]]:
        project = InfraHelper.get_by_id_like(Project, project)
        if not project:
            return []

        checklists = self.repo.checklist.get_all_by_project(project, archive_visible_since)
        return [checklist.api_response() for checklist in checklists]
