...
class ProjectAssignedUser(BaseDbModel, table=True):
    ...
    board_seen_seq: int = Field(default=0, nullable=False, sa_column_kwargs={"server_default": "0"})
    card_unread_baseline_seq: int = Field(default=0, nullable=False, sa_column_kwargs={"server_default": "0"})

    def notification_data(self) -> dict[str, Any]:
        return {}

    def _get_repr_keys(self) -> list[str | tuple[str, str]]:
        return ["project_id", "user_id", "starred", "last_viewed_at", "view_count", "board_seen_seq", "card_unread_baseline_seq"]
