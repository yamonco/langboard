...
for (const width of [390, 1440]) {
    for (const embedded of [false, true]) {
        test(`notification navigation preserves each destination at viewport ${width}, embedded ${embedded}`, async ({ page }) => {
            await page.setViewportSize({ width, height: 900 });
            await page.goto(`/src/components/Header/notification-navigation.fixture.html${embedded ? "?embedded" : ""}`);
