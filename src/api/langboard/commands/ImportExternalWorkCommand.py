...
MAX_IMPORT_BUNDLE_BYTES = 64 * 1024 * 1024


def load_bounded_bundle(path: Path, max_bytes: int = MAX_IMPORT_BUNDLE_BYTES) -> ExternalWorkBundle:
    """Load one explicitly bounded bundle without an unbounded `read_text()` allocation."""

    payload = bytearray()
    with path.open("rb") as stream:
        while chunk := stream.read(min(1024 * 1024, max_bytes + 1 - len(payload))):
            payload.extend(chunk)
            if len(payload) > max_bytes:
                raise ValueError(f"bundle exceeds the {max_bytes} byte input limit")
    return ExternalWorkBundle.model_validate_json(payload)
...
class ImportExternalWorkCommand(BaseCommand):
    ...
    def description(self) -> str:
        return "Import a bounded provider-neutral work bundle with restartable checkpoints"
    ...
    def execute(self, bundle_file: str, options: ImportExternalWorkCommandOptions) -> None:
        ...
        bundle = load_bounded_bundle(path)
