...
class GZipDecompressMiddleware(BaseMiddleware):
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        with TemporaryFile() as decompressed_body:
            while True:
                ...
                compressed_body = message.get("body", b"")
                ...
                if not message.get("more_body", False):
                    ...
                    if not decompressor.eof:
                        await self._send_invalid_body(scope, receive, send)
                        return
                    decompressed_body.write(tail)
                    break

            decompressed_body.seek(0)
            replayed_size = 0
    ...
    @staticmethod
    async def _send_invalid_body(scope, receive, send) -> None:
        response = JsonResponse(
